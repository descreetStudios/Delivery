		...
		<div class="top-4 left-1/2 z-10 absolute w-105 -translate-x-1/2">
			<div class="flex items-center bg-bg-surface shadow-lg px-4 rounded-full h-12">
				...
				<div class="justify-center items-center invert size-5 text-bg-surface">
					...
					<svg
						v-else
						class="size-auto scale-85 animate-spin"
						viewBox="0 0 64 64"
						fill="none"
						xmlns="http://www.w3.org/2000/svg"
					>
				...
				<input
					...
					type="text"
					maxlength="100"
					placeholder="Look for a place to eat"
...
watch(query, async (q) => {
	...
	debounceTimer = setTimeout(async () => {
		...
		const filtered = res.filter(item => civicTypes.includes(item.description));
